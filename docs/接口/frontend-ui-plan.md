...
- 左侧为全局导航区，固定展示写邮件、收件箱，已发送，已删除，垃圾邮箱。
- 左下角展示当前用户头像和名字，头像用名字取首字母或者首字。
- 点击用户头像弹出用户菜单，菜单包括退出登录，修改密码，设置。
...
## 4.布局方案 （如果是进入详情，可以在右侧顶部靠左边显示<-- 表示返回）
...
### 8.5 设置页 如果需要枚举的选项，按照接口里面的枚举值下拉选项给用户选
